...
void eval(char *cmdline)
{
	sigset_t mask;
	char *argv[MAXARGS];
	int bg = parseline(cmdline, argv); //bg면 1리턴, fg면 0리턴
	pid_t pid;
	...
	if(argv[0] == NULL)
  	...
  	if(!builtin_cmd(argv)){ //builtin 명령어가 아닌 경우 fork
		...
		if(pid == 0){ //자식 프로세스
			...
			addjob(jobs, pid, bg?BG:FG, cmdline); //add
			sigprocmask(SIG_UNBLOCK, &mask, NULL);
			if(bg) //bg인 경우 출력
				printf("[%d] (%d) %s",pid2jid(pid) ,pid, cmdline);
			else{ //fg일 경우 다른 걸 못 하게 함
				waitfg(pid);
			}
...
int builtin_cmd(char **argv)
{
	...
	if(!strcmp("quit", argv[0])) //명령어에 quit가 도착한 경우
		exit(0);
